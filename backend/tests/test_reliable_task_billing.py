...
async def test_account_level_active_task_limit_blocks_parallel_paid_work():
    ...
    # bid_draft, polish, bid_wizard_qa, bid_wizard_index, bid_wizard_write
    # (every new billable kind must join the active-task gate too).
    db.execute = AsyncMock(
        side_effect=[_scalar(1), _scalar(0), _scalar(0), _scalar(0), _scalar(0), _scalar(0), _scalar(0)]
    )
