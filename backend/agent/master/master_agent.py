...
class MasterAgent:
    ...
    async def run(self, todo_service, session_id: str) -> dict:
        """
        执行主代理工作流程.
            ...
            try:
                ...
                if detect_anomaly(result, todo):
                    if retry_count < self.max_retries:
                        ...
                        await todo_service.update_todo_status(
                ...
                await todo_service.increment_completed_todos(self._session_id)
    ...
    async def _merge_results(self, todo_service) -> dict:
        """合并所有子代理结果."""
        todos = await todo_service.get_session_todos(self._session_id)
